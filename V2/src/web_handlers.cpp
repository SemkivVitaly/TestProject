/**
 * web_handlers.cpp — обработчики HTTP и регистрация маршрутов. Интерфейс на русском.
 */
#include "config.h"
#ifdef WEB_SERVER
...
static void handleRoot() {
    sendHtml(F(
        ...
        "<p><a href='/status'>Общий статус (JSON)</a> | <a href='/api/link'>Метрики канала (JSON)</a> | <a href='/bridge'>Интерфейс Bridge</a></p>"
        "<script>"
...
static void handleParamsPage() {
    String html = F(
        ...
        "document.getElementById('conn').innerHTML='Подключение: '+(j.connected?'<span class=ok>Да</span>':'<span class=no>Нет</span>')+' | RX: '+j.packets_rx+' TX: '+j.packets_tx+' | Задержка: '+(j.connected?j.latency_ms:'—')+' мс | Потери: '+j.packet_drops+' ('+j.packet_loss_pct+'%)';"
...
static void handleApiSettingsGet() {
    String s;
    s += F("{\"esp32_mode\":1,\"ssid\":\""); s += SSID;
    s += F("\",\"wifi_pass\":\""); s += PASSWD;
    s += F("\",\"wifi_chan\":1,\"ap_ip\":\"192.168.2.1\",\"wifi_en_gn\":1,\"radio_dis_onarm\":0,\"ant_use_ext\":0,\"ip_sta\":\"\",\"ip_sta_gw\":\"\",\"ip_sta_netmsk\":\"\",\"wifi_hostname\":\"Bridge_ESP32\",\"gpio_tx\":");
    s += SERIAL_TXPIN; s += F(",\"gpio_rx\":"); s += SERIAL_RXPIN;
    s += F(",\"gpio_rts\":0,\"gpio_cts\":0,\"rts_thresh\":64,\"proto\":4,\"baud\":"); s += UART_BAUD;
    s += F(",\"ltm_per_packet\":1,\"trans_pack_size\":256,\"serial_timeout\":50,\"rep_rssi_dbm\":0}");
    sendJson(s);
}

static void handleApiSettingsPost() {
    sendJson(F("{\"msg\":\"Настройки приняты. Перезагрузка через 1 с...\"}"));
    delay(1000);
    ESP.restart();
}

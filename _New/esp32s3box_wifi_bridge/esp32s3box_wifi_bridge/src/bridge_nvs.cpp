/**
 * bridge_nvs.cpp — загрузка/сохранение настроек моста в NVS (Preferences).
 * При первом запуске используются значения по умолчанию из config.h.
 */
#include "config.h"
...
/** Подставить значения по умолчанию из config.h. */
static void setDefaults(void) {
    bridge_nvs_config.wifi_mode = 1; /* AP */
#ifdef MODE_STA
    bridge_nvs_config.wifi_mode = 2; /* STA */
#endif
    ...
    bridge_nvs_config.proto = 4; /* MAVLink */
}

/** Извлечь строковое значение ключа из JSON (упрощённый разбор). */
...
/** Извлечь целое значение ключа из JSON. */
...
bool saveBridgeConfig(void) {
    if (!prefs.begin(NVS_NAMESPACE, false))
        return false;
    bool ok = prefs.putBytes("cfg", &bridge_nvs_config, sizeof(bridge_nvs_config_t)) == sizeof(bridge_nvs_config_t);
    prefs.end();
    return ok;
}
...
bool setBridgeConfigFromJson(const char* json) {
    ...
    int ival;
    if (jsonGetInt(json, "esp32_mode", &ival) && (ival == 1 || ival == 2))
        bridge_nvs_config.wifi_mode = (uint8_t)ival;
    ...
    if (jsonGetInt(json, "wifi_chan", &ival) && ival >= 1 && ival <= 13)
        bridge_nvs_config.wifi_chan = (uint8_t)ival;
    jsonGetString(json, "ap_ip", bridge_nvs_config.ap_ip, BRIDGE_NVS_APIP_LEN);
    if (jsonGetInt(json, "baud", &ival) && ival > 0)
        bridge_nvs_config.baud = (uint32_t)ival;
    if (jsonGetInt(json, "gpio_tx", &ival))
        bridge_nvs_config.gpio_tx = (int8_t)ival;
    if (jsonGetInt(json, "gpio_rx", &ival))
        bridge_nvs_config.gpio_rx = (int8_t)ival;
    if (jsonGetInt(json, "proto", &ival))
        bridge_nvs_config.proto = (uint8_t)ival;

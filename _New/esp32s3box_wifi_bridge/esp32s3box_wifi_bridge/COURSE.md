...
**Сборка проекта с нуля с полным кодом.**  
Если нужно не только понять готовый проект, но и **собрать его по шагам от пустой папки до работающего веб-интерфейса**, с полным кодом каждого файла и пояснениями — используйте отдельный документ **[TUTORIAL_FROM_SCRATCH.md](TUTORIAL_FROM_SCRATCH.md)**. В нём пошагово: создание проекта и platformio.ini, config.h, main.cpp (целиком), bridge.h и bridge.cpp (целиком), затем bridge_nvs, mavlink_state, логи, web_handlers и скрипт встраивания веб-страницы. В этом курсе (COURSE.md) ниже приведён **полный код setup() и loop()** из main.cpp и таблица, что делает каждый вызов в loop().
...
**Полный код setup() и loop() из main.cpp — как это реально написано в проекте.**

Ниже — код от начала до конца. Каждая часть отвечает за конкретный шаг запуска (setup) или один «тик» основного цикла (loop).

**1. Функция setup() — выполняется один раз при включении платы**

```cpp
void setup() {
    delay(500);                                    // Даём питанию стабилизироваться
    Serial.begin(115200);                           // Скорость отладочного порта (монитор 115200)
    debug.print(F("\nWiFi bridge "));
    debug.println(VERSION);
    espLogPrintf("[boot] WiFi bridge %s", VERSION);

    loadBridgeConfig();                             // Читаем NVS → bridge_nvs_config (или defaults)
    ...
    SerialUART.begin(baud, SERIAL_PARAM, (int)rxPin, (int)txPin);  // UART к автопилоту
    ...
    if (bridge_nvs_config.wifi_mode == 2) {         // Режим STA — подключаемся к роутеру
        WiFi.mode(WIFI_STA);
    ...
    } else {                                        // Режим AP — плата раздаёт Wi‑Fi
        WiFi.mode(WIFI_AP);
    ...
    bridgeSetup();                                  // TCP-сервер 8880, UDP 14550

#ifdef WEB_SERVER
    LittleFS.begin(true);
    webSetup(webServer);                            // Регистрация всех маршрутов /, /params, /api/*
...
**2. Функция loop() — вызывается бесконечно; полный код без сокращений**

```cpp
void loop() {
#ifdef OTA_HANDLER
    ArduinoOTA.handle();                            // Проверить, не идёт ли обновление по Wi‑Fi
#endif

#ifdef WEB_SERVER
    webServer.handleClient();                       // Обработать один HTTP-запрос (/, /api/status и т.д.)
    mavlinkCheckDisconnect();                       // Если > 5 с не было HEARTBEAT — mavlinkConnected = false
    ...
    bridgeAcceptClient();                           // Если есть новый TCP-клиент — принять в свободный слот
    bridgePollDisconnects();                        // Удалить отключённых TCP, сбросить UDP по таймауту
    bridgePollNetworkToUart();                      // Данные из TCP (и BT) → записать в SerialUART

    while (SerialUART.available()) {                 // Всё, что пришло с автопилота по UART
        lenFromUART = 0;
        while (SerialUART.available() && lenFromUART < BUFFERSIZE - 1)
            bufFromUART[lenFromUART++] = SerialUART.read();

        if (lenFromUART == 0) break;
        bridgeLogSetLastRx(bufFromUART, lenFromUART);       // Образец для единого лога
        mavlinkProcessBytes(bufFromUART, lenFromUART);      // Парсинг HEARTBEAT, PARAM_VALUE и т.д.
        bridgeSendUartToNetwork(bufFromUART, lenFromUART);  // Те же байты — всем TCP/UDP клиентам
    }

    delay(1);                                       // Не нагружать CPU на 100%
}
```

**Как это работает по шагам в loop():**

| Шаг | Вызов | Что происходит |
|-----|--------|----------------|
| 1 | `webServer.handleClient()` | Если браузер запросил страницу или API — формируется ответ (HTML или JSON) и отправляется. Без этого веб-интерфейс не отвечает. |
| 2 | `mavlinkCheckDisconnect()` | Сравнивает `millis()` и `lastHeartbeatMs`. Если прошло больше 5 с — связь с автопилотом сбрасывается (`mavlinkConnected = false`). |
| 3 | `bridgeAcceptClient()` | Проверяет `tcpServer.hasClient()`. Если есть новое подключение (например Mission Planner) — принимает его в первый свободный слот `tcpClients[]`. |
| 4 | `bridgePollDisconnects()` | Проходит по `tcpClients[]`, отключённых закрывает и освобождает слот; для UDP сбрасывает «известного клиента», если давно не было пакетов. |
| 5 | `bridgePollNetworkToUart()` | Читает данные из каждого подключённого TCP-клиента (и из BT) и пишет их в `SerialUART` — так команды от GCS попадают к автопилоту. |
| 6 | `while (SerialUART.available())` | Пока в буфере UART есть байты: читаем их в `bufFromUART`, сохраняем образец в лог, разбираем MAVLink для статуса/параметров, затем те же байты отправляем всем сетевым клиентам — так телеметрия попадает в GCS. |
| 7 | `delay(1)` | Короткая пауза, чтобы не крутить цикл без паузы (снижение нагрузки на CPU). |
